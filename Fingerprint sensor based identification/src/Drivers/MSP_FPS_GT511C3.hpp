...
#ifndef MSP_FPS_GT511C3_hpp
#define MSP_FPS_GT511C3_hpp


/*
	Command_Packet represents the 12 unsigned char command that we send to the finger print scanner
*/
class Command_Packet
{
	...
	private:
		static const unsigned char COMMAND_START_CODE_1 = 0x55;	// Static unsigned char to mark the beginning of a command packet	-	never changes
		static const unsigned char COMMAND_START_CODE_2 = 0xAA;	// Static unsigned char to mark the beginning of a command packet	-	never changes
		static const unsigned char COMMAND_DEVICE_ID_1 = 0x01;	// Device ID unsigned char 1 (lesser unsigned char)							-	theoretically never changes
		static const unsigned char COMMAND_DEVICE_ID_2 = 0x00;	// Device ID unsigned char 2 (greater unsigned char)							-	theoretically never changes
		unsigned char command[2];								// Command 2 bytes

		unsigned long int _CalculateChecksum();						// Checksum is calculated using unsigned char addition
		unsigned char GetHighByte(unsigned long int w);
		unsigned char GetLowByte(unsigned long int w);
};
...
class Response_Packet
{
	public:
		class ErrorCodes
		{
			public:
				...
				static Errors_Enum ParseFromBytes(unsigned char high, unsigned char low);
		};
	...
	private:
		bool CheckParsing(unsigned char b, unsigned char propervalue, unsigned char alternatevalue, char* varname, bool UseSerialDebug);
		unsigned long int CalculateChecksum(unsigned char* buffer, int length);
		unsigned char GetHighByte(unsigned long int w);
		unsigned char GetLowByte(unsigned long int w);
};

// Data Mule packet for receiving large data(in 128 unsigned char pieces) from the FPS
...
class FPS_GT511C3
{
 ...
 public:
	...
	FPS_GT511C3(unsigned char rx, unsigned char tx);
	...
	// Not implemented due to memory restrictions on the arduino
	// may revisit this if I find a need for it
	//int SetTemplate(unsigned char* tmplt, int id, bool duplicateCheck);
	...
	void serialPrintHex(unsigned char data);
	void SendToSerial(unsigned char data[], int length);
...
private:
	 ...
	 //unsigned char pin_RX,pin_TX;
	 //SoftwareSerial _serial;
};


#endif //MSP_FPS_GT511C3_hpp
